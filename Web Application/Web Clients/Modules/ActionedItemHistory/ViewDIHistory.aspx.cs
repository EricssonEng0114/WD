...
{
    public partial class ViewDIHistory : System.Web.UI.Page
    {
        ...
        private void LoadItemDataTableInfo()
        {
            try
            {
                ...
                if (ValidateSessionValue())
                {
                    ...
                    DataTable dtDetail = CommonFunction.GetDetailItemInfo("DI", Session["selClientDBConnStr"].ToString(), curDecision, Session["s_CurSelectedRejectReason"].ToString());
                ...
                {
                    //Redirect to index page
                    Logger.Write(false, LogCallerID.ActionedItemHistory, curSelectedClient, "View Actioned Item History", "Invalid session value for DI actioned item history.", LogEventType.Error, userID);
                    Response.Redirect("/History", false);
                    Context.ApplicationInstance.CompleteRequest();
                }
            }
        ...
        private void DisplayTwoItemsInfo()
        {
            if (Convert.ToInt16(Session["curTransItemsCount"]) > 0)
            {
                ...
                if (Convert.ToBoolean(itemTop["SystemGen"]))
                {
                    ...
                    // flipImage("{0}", "{1}", "{2}");
                    ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), "flipTopImage", String.Format("flipImage('{0}', '{1}', '{2}');", "ADI", "TOP", "#DIImgViewerTop"), true);
                }
                ...
                if (Convert.ToInt16(Session["topItemPtr"]) + 1 < Convert.ToInt16(Session["curTransItemsCount"]))
                {
                    ...
                    if (Convert.ToBoolean(itemBottom["SystemGen"]))
                    {
                        ...
                        // flipImage("{0}", "{1}", "{2}");
                        ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), "flipBottomImage", String.Format("flipImage('{0}', '{1}', '{2}');", "ADI", "BOT", "#DIImgViewerBottom"), true);
                    }
                }
                else
                {
        ...
        private void DisplaySelItemFieldsAndImage(bool isTopItem)
        {
            ...
            if (!noAif)
            {
                txtA1stName.InnerText = itemRow["firstName"].ToString();//.Split("|".ToCharArray());
                txtA2ndAcctName.InnerText = itemRow["secondName"].ToString();//.Split("|".ToCharArray());
                txtA3rdAcctName.InnerText = itemRow["thirdName"].ToString();//.Split("|".ToCharArray());
                txtA4thAcctName.InnerText = itemRow["fourthName"].ToString();//.Split("|".ToCharArray());
            }
            else
            {
        ...
        protected void btnSelectBottom_Click(object sender, EventArgs e)
        {
            ...
            Session["topItemSelected"] = false;
            //DisplaySelItemFieldsAndImage(false);
            DisplayTwoItemsInfo();
            Session["curItemPtr"] = Convert.ToInt16(Session["topItemPtr"]) + ((Convert.ToBoolean(Session["topItemSelected"])) ? 0 : 1);
        }
